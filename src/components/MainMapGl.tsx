...
const MainMap = (props: { ws: WebSocket; region: any }) => {
  console.log('props:',props.ws === null, props.ws)
  //== Piece of Redux =======================================
  ...
  if (!flagOpen && !propsOpen) {
    ...
    if (!props.region && dateMap.length) homeRegion = dateMap[0].region.num;
  ...
  const MapGl = (props: { pointa: any; pointb: any }) => {
    ...
    const MakeСollectionRoute = () => {
      for (let i = 0; i < massroute.length; i++) {
        if (massroute[i].starts === CodingCoord(pointAa)) {
          coordStop.push(DecodingCoord(massroute[i].stops)); // исходящие связи
          coordStart.push(pointAa);
        }
        if (massroute[i].stops === CodingCoord(pointAa)) {
          coordStartIn.push(DecodingCoord(massroute[i].starts)); // входящие связи
