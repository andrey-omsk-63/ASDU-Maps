...
const MainMap = (props: {
  ...
  if (openSetBind && pointAaIndex < 0 && pointBbIndex < 0) setOpenSetBind(false); // отработка Esc из RouteBind

  //=== обработка Esc ======================================
  const escFunction = React.useCallback(
    (event) => {
      if (event.keyCode === 27)
        if (pointAa || flagBind || flagRoute || flagPusk || openSetVertForm || openSetWaysForm)
          ZeroRoute(false);
    },
        ...
        <YMaps query={{ apikey: MyYandexKey, lang: 'ru_RU' }}>
          <Map
            ...
            {openSetVertForm && pointAaIndex >= 0 && triggerForm && (
              <MapVertexForma setOpen={SetOpenSetVertForm} idx={pointAaIndex} forma={VertexForma} />
            )}
            {openSetVertForm && pointAaIndex >= 0 && !triggerForm && (
              <MapVertexForma setOpen={SetOpenSetVertForm} idx={pointAaIndex} forma={VertexForma} />
            )}
            ...
            {openSetDelete &&
              DelVertexOrPoint(openSetDelete, massdk, massroute, idxDel, handleCloseDel)}
            {openSetRevers && (
              <MapReversRoute
