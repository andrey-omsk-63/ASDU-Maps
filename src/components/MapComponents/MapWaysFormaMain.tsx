...
const MapWaysFormaMain = (props: {
  ...
  const [currencyOpp, setCurrencyOpp] = React.useState(
    props.maskForm.opponent === '' ? '0' : props.massDir.indexOf(props.maskForm.opponent),
  );
  ...
  const OpponentDirect = () => {
    ...
    return (
      <Box sx={{ marginTop: 1.5, marginLeft: -0.5, marginRight: -0.5 }}>
        <Grid container>
          <Grid item xs={1.2} sx={{ fontSize: 14, marginTop: 0.4, textAlign: 'center' }}>
            <b>{metka}</b>
          </Grid>
  ...
  return (
    <>
      ...
      {StrTab('Интенсивность(т.е./ч.)', WaysInput(0, massForm.intensTr, SetIntensTr, 10000))}
      {StrTab('Дисперсия пачки(%)', WaysInput(0, massForm.dispers, SetDispers, 100))}
      {StrTab('Длинна перегона(м)', WaysInput(0, massForm.peregon, SetPeregon, 99999))}
      ...
      {OpponentDirect()}
      <Box sx={{ fontSize: 12, marginTop: 1.5 }}>Выберите зелёные фазы для данного направления</Box>
